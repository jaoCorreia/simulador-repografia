...
function renderOverlay(state) {
  ...
  $('#footer-right').innerHTML = inLobby ? '<span class="key-mini">F</span> SOLTAR <span class="footer-separator">/</span> R·T INCLINAR · RODA APROXIMAR' : menu ? '<span class="key-mini">↔</span> MOVA O MOUSE PARA OLHAR' : '<span class="key-mini">ESC</span> PAUSAR <span class="footer-separator">/</span> CLIQUE OU ESPAÇO PARA APERTAR';
...
function frame(now) {
  ...
  const s = game.state;
  world?.update(dt, { phase: sessionScreen === 'lobby' ? 'lobby' : sessionScreen === 'welcome' ? 'menu' : s.phase, force: sessionScreen === 'game' ? s.force : 0, papers: sessionScreen === 'game' ? s.order?.papers || 12 : 12, alignment: sessionScreen === 'game' ? s.alignment : 0, pressPulse, celebration, lookX: look.x, lookY: look.y, paused: helpOpen || document.hidden || (sessionScreen === 'game' && s.paused), elapsed, jammed: s.phase === 'jam' });
  lobbyUI.tick(dt);
  updateUI(s);
